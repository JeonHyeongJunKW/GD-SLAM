...
//numpy를 만들 때 사용하는 클래스, cv랑 PyObject가 필요하다.
class NumpyAllocator : public MatAllocator
{
public:
#if ( CV_MAJOR_VERSION < 3)//Opencv버전에 따라서 다른 방식을 취한다.
    NumpyAllocator() {}
    ~NumpyAllocator() {}
...
{//pyobject를 mat으로 바꿉니다.
    ...
    if( m.data )
    {
        ...
        m.addref();//ref카운터를 올립니다.
        Py_INCREF(o);//객체의 ref카운터를 올립니다.
