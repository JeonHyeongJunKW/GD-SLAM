     ...
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth), mIsKeyFrame(false), mImDepth(imDepth)
{
    ...
    if(mbInitialComputations)
    {
        ComputeImageBounds(imGray);//이미지의 경계를 계산합니다.
     ...
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth), mIsKeyFrame(false), mImDepth(imDepth), mImRGB(imRGB)
{
    ...
    // Delete those ORB points that fall in Mask borders (Included by Berta)
    cv::Mat Mask_dil = imMask.clone();//입력받은 마스크입니다.
    int dilation_size = 15;
    cv::Mat kernel = getStructuringElement(cv::MORPH_ELLIPSE,
                                        cv::Size( 2*dilation_size + 1, 2*dilation_size+1 ),
                                        cv::Point( dilation_size, dilation_size ) );
    ...
    {
        int val = (int)Mask_dil.at<uchar>(mvKeys[i].pt.y,mvKeys[i].pt.x);
        if (val == 1)//정적인 영역이면 키포인트 및 descriptor에 넣어라.
        {
            _mvKeys.push_back(mvKeys[i]);
            _mDescriptors.push_back(mDescriptors.row(i));
        }
    ...
    UndistortKeyPoints();//멤버변수로 저장된 키포인트들을 보정한다.

    ComputeStereoFromRGBD(imDepth);//멤버키포인트에 대해서 depth를 저장한다.

    mvpMapPoints = vector<MapPoint*>(N,static_cast<MapPoint*>(NULL));//맵포인트 저장할 벡터 선언(왜 mvp일까)
    mvbOutlier = vector<bool>(N,false);//아웃라이어여부 저장변수, 일단 false로 초기화한다.

    // This is done only for the first Frame (or after a change in the calibration)
    if(mbInitialComputations)//초기 프레임에 대해서 검사되는 단계
    {
        ComputeImageBounds(imGray);

        mfGridElementWidthInv=static_cast<float>(FRAME_GRID_COLS)/static_cast<float>(mnMaxX-mnMinX);//세로 48개, 길이로 나눔. 그냥 inverse 값인듯
        mfGridElementHeightInv=static_cast<float>(FRAME_GRID_ROWS)/static_cast<float>(mnMaxY-mnMinY);//가로 64개 인데, 이걸 갯수로 나눈다.
     ...
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    ...
    AssignFeaturesToGrid();//피처 매칭을 가속화하기 위해 그리드에 키포인트를 할당합니다(생성자에서 호출됨).
}

void Frame::AssignFeaturesToGrid()
{
    int nReserve = 0.5f*N/(FRAME_GRID_COLS*FRAME_GRID_ROWS);//키포인트의 수/전체 그리드의 수의 절반
    for(unsigned int i=0; i<FRAME_GRID_COLS;i++)
        for (unsigned int j=0; j<FRAME_GRID_ROWS;j++)
            mGrid[i][j].reserve(nReserve);//각 그리드당 보존할 feature수의 절반으로한다.

    for(int i=0;i<N;i++)
    {
        const cv::KeyPoint &kp = mvKeysUn[i];//i번째 키포인트를 가져오고

        int nGridPosX, nGridPosY;
        if(PosInGrid(kp,nGridPosX,nGridPosY))
            mGrid[nGridPosX][nGridPosY].push_back(i);//일정 그리드에 저장한다.
    }
}
...
{
    posX = round((kp.pt.x-mnMinX)*mfGridElementWidthInv);//몇번째 그리드에 속하는지 계산가능
...
{
    if(mDistCoef.at<float>(0)!=0.0)//왜곡보정에 의한 이미지 경계출력연산을 한다.
    {
...
void Frame::ComputeStereoFromRGBD(const cv::Mat &imDepth)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        const float d = imDepth.at<float>(v,u);//distortion해서 나온 좌표에 대해서, depth를 사용한다. (1. undistortion -> 2. use depth)
